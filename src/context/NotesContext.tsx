...
export function NotesProvider({
  ...
  const fetchNotes = async (collectionName2 = collectionName) => {
    try {
      ...
      // Order notes by updatedAt (or createdAt if missing) in descending order (newest first)
      ...
      if (note.id) {
        ...
        try {
          ...
          if (noteSnap.exists()) {
            ...
            console.log(
              `Note with ID ${note.id} doesn't exist in Firestore, creating new document`
            );
  ...
  const addReply = async (
    ...
    try {
      console.log(
        `Attempting to add reply to note ID: ${noteId} in collection: ${collectionName2}`
      );
      ...
      console.error(
        `Error adding reply to note ID: ${noteId} in collection: ${collectionName2}:`,
        error
      );
      throw error;
    }
  ...
  useEffect(() => {
    ...
    const loadInitialNotes = async () => {
      try {
        ...
        console.log(
          `Successfully fetched initial notes from collection: ${collectionName}`
        );
      } catch (error) {
        console.error(
          `Error fetching initial notes from collection: ${collectionName}`,
          error
        );
      }
    };
  ...
  const memoizedFetchReplies = useCallback(fetchReplies, []);
  const memoizedDeleteReply = useCallback(deleteReply, []);
